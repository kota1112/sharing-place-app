...
export default function PlaceEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPhotos((data.photo_urls || []).map((url, idx) => ({ url, key: `${idx}-${url}` })));
      } catch (e) {
  ...
  async function onDeletePhoto(urlOrId) {
    try {
      ...
      setPhotos((list) => list.filter((p) => p.url !== urlOrId && p.key !== urlOrId));
    } catch (e) {
      ...
      <div className="mx-auto max-w-4xl px-4 pt-10 pb-28">
        ...
        {!loading && !err && (
          <form onSubmit={onSubmit} className="space-y-6">
                ...
                <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {photos.map((p) => (
                    <li key={p.key || p.url} className="rounded-xl border">
                      ...
                      <div className="truncate px-3 py-2 text-xs text-gray-500">{p.url}</div>
                    </li>
                  ))}
                </ul>
              )}
            ...
            {/* 追加アップロード */}
            <section>
              <div className="mb-2 text-sm font-medium">写真を追加（複数可）</div>
            ...
            <div className="sticky bottom-0 left-0 right-0 z-10 -mx-4 border-t bg-white/95 px-4 py-3 backdrop-blur supports-[backdrop-filter]:bg-white/70">
              <div className="mx-auto flex max-w-4xl items-center justify-end gap-3">
                {err && <span className="truncate text-sm text-red-600">{err}</span>}
                <button
                  type="submit"
