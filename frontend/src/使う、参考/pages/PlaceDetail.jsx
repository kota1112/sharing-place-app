  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const { Map } = canImport ? await g.maps.importLibrary("maps") : { Map: g.maps.Map };
        ...
        if (canImport) {
          ...
          adv.addListener("gmp-click", () => openInfo(place, adv, map, infoRef.current));
          markerRef.current = adv;
        } else {
          const m = new g.maps.Marker({ map, position: center, title: place.name || "" });
          m.addListener("click", () => openInfo(place, m, map, infoRef.current));
          markerRef.current = m;
        }
      } catch (e) {
  ...
  if (!hasCoords) {
    return (
      <section className="rounded-2xl border bg-white p-5 shadow-sm">
        ...
        <p className="text-gray-600">この場所の地図情報は登録されていません。</p>
        {gmapsUrl && (
          <a
            href={gmapsUrl}
            target="_blank"
...
function buildMapsUrl(p) {
  ...
  if (gpid) {
    return `https://www.google.com/maps/place/?q=place_id:${encodeURIComponent(gpid)}`;
  }
...
function buildInfoHtml(p) {
  ...
  const addr =
    ...
    "";
  const addrHtml = addr
    ? `<div style="font-size:12px;color:#374151;margin-top:2px;">${escapeHtml(addr)}</div>`
    : "";

  const desc = (p.description || "").trim();
  const descShort = desc.length > 60 ? `${desc.slice(0, 60)}…` : desc;
  const descHtml = descShort
    ? `<div style="font-size:12px;color:#111827;margin-top:6px;">${escapeHtml(descShort)}</div>`
    : "";
